...
int deserializeme(){
    ...
    if (Serial::DeserializeFromFile(DATAFOLDER + "/ciphertext1.txt", ct, SerType::BINARY) == false) {
        std::cerr << "Could not read the ciphertext" << std::endl;
        return 1;
    }

    std::cout << "Deserialized the ciphertext ." << std::endl;

    CT pat(0);

    Ciphertext<DCRTPoly> pattern;
    if (Serial::DeserializeFromFile(DATAFOLDER + "/pattern.txt", pat, SerType::BINARY) == false) {
        std::cerr << "Could not read the ciphertext" << std::endl;
        return 1;
    }
    std::cout << "pattern has been deserialized." << std::endl;
    ...
    vecCT eresult = encrypted_search(cc, pk, epat, etxt, p);
...
int serializeme()
{
   ...
   //}
    ...
    if (!Serial::SerializeToFile(DATAFOLDER + "/" + "pattern.txt", epat[0], SerType::BINARY)) {
    ...
    if (!Serial::SerializeToFile(DATAFOLDER + "/" + "ciphertext1.txt", etxt[0], SerType::BINARY)) {
