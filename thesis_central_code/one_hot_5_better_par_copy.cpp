...
//homolog is totally wrong and can be deleted, just need pattern match enc only
   ...
   //homolog(ct_genome, ct_pattern, homolog_result, homo, cc);
    ...
    CT **pattern_match_array = new CT*[genome_len-pattern_len+1 - num_wildcards];
    for (int i = 0; i < genome_len-pattern_len+1- num_wildcards; i++){
        pattern_match_array[i] = new CT[4];
    }

    vector<vecCT> pattern_match_enc_result_homo;
    pattern_match_enc_homolog(ct_genome, ct_pattern, pattern_match_enc_result_homo, one_hot_pattern, cc, homo, pattern_match_array);
    printf("\nenter roate\n");

    //rotate
    CT **rot2 = new CT*[4];
    for (int i = 0; i < 4; i++){
        rot2[i] = new CT[genome_len-pattern_len+1-num_wildcards];
    }
    ...
    for (int i = 0; i < 4; i++){
        vecCT temp;
        for (int j = 0; j < genome_len - pattern_len + 1 - num_wildcards; j++){
    ...
    CT* percent_match_homo_array = new CT[genome_len - pattern_len + 1 - num_wildcards];

    vecCT percent_match_result_for_homolog;
    precent_match_homolog(pattern_match_enc_result_homo, cc, percent_match_result_for_homolog, homo, ct_pattern, percent_match_homo_array);
    //populate the vector
    for (int i = 0; i < genome_len - pattern_len + 1 - num_wildcards; i++){
    ...
    get_the_homolog(percent_match_result_for_homolog, get_the_homolog_result, homo, cc, sk, ct_genome, ct_pattern, homo_array);
